...
contract GetPrice is Test {
    ...
    /// --- STORAGE
    ////////////////////////////////////////////////////
    string jsonPathInput = "/test/json/GetPrice_Input.json";
    string jsonPathOutput = "/test/json/GetPrice_Output.json";
    ...
    function test_GetPrice() public {
        ...
        uint256 lenSWAP = 1; // Only one price
        ...
        for (uint256 i; i < lenA; i++) {
            ...
            for (uint256 j; j < lenAMO; j++) {
                ...
                for (uint256 k; k < lenSWAP; k++) {
                    output[i][j][k] = tx_(values.A[i], values.AMO_PCT[j]);
                }
            }
        }
        writeInJson(output);
    ...
    function tx_(uint256 aFactor, uint256 amoPct) public isolate returns (uint256) {
    ...
    function getPrice() public view returns (uint256) {
        return pool.get_dy(int128(1), int128(0), WAD);
    }
